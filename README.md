# Nashville House Cleaning Directory

A local directory platform connecting Nashville residents with trusted house cleaning services.

## Overview

This platform serves two primary audiences:

1. **Nashville Residents** - Find, compare, and book reliable house cleaning services
2. **Cleaning Businesses** - Grow your customer base with qualified local leads

## Documentation

- [Marketing Plan](/docs/MARKETING_PLAN.md) - Comprehensive marketing strategy for launching and growing the directory
- [Tech Stack Decisions](./docs/TECH_STACK.md)
- [Authentication Strategy](./docs/AUTHENTICATION.md) - Complete auth implementation guide for 3 user types
- [Pricing Page](./docs/PRICING_PAGE.md) - Three-tier pricing model and conversion strategy
- [Admin Dashboard](./docs/ADMIN_DASHBOARD.md) - Admin panel design, layout, and features
- [Color Palette](./docs/COLOR_PALETTE.md) - Design system and color usage guide
- [Database Schema](./docs/DATABASE_SCHEMA.md)
- [Brainstorm Sessions](./docs/BRAINSTORM.md)

## For AI Development
When building this application, review all files in `/docs` first.

## Value Proposition

### For Consumers
- Find trusted Nashville house cleaning services in minutes
- Compare local providers and read reviews
- Book with confidence—all in one place

### For Cleaning Businesses
- Get discovered by customers actively searching for services
- Qualified local leads without the high fees of national platforms
- Better economics than Thumbtack, Angi, or HomeAdvisor

## Market Opportunity

- Nashville metro area: 2M+ population
- One of the fastest-growing cities in the US
- Strong demand for house cleaning services
- Opportunity to compete with national platforms through local focus

## Getting Started

### Setup Instructions

See [Setup Guide](./docs/SETUP.md) to:
- Configure environment variables
- Set up Supabase database
- Generate NextAuth secrets
- Run the development server

### Development Strategy

See the [Marketing Plan](./docs/MARKETING_PLAN.md) for detailed strategies on:
- Reaching Nashville residents
- Recruiting cleaning businesses
- Launch timeline and budget
- Success metrics and KPIs

## Contact

For questions or collaboration opportunities, please reach out.

---

**Built for Nashville. By Nashville.**
